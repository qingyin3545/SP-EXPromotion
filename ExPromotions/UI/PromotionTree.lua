...
function PlaceUnit(pUnit)
  ...
  PlacePromotions(pUnit, sDisplayClass, iUnitBoxX + iUnitBoxSizeX, iCentreLine,sDisplayBasePromotion)
end

function PlacePromotions(pUnit, sCombatClass, iBaseX, iBaseY,sBasePromotion)
...
function DrawPromotionButton(iX, iY, pUnit, sPromotion, sCombatClass)
  ...
  if (sPromotion == "PROMOTION_SPARTAN300_FIGHT_TO_THE_END" and sCombatClass == "UNITCOMBAT_MELEE") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("斯巴达[COLOR_POSITIVE_TEXT]专属[ENDCOLOR]晋升!")
  end
  --舰炮齐射
  if (sPromotion == "PROMOTION_SHIPBORNE_GUN_SALVO" and sCombatClass == "UNITCOMBAT_NAVALRANGED") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("拥有后勤补给[ICON_PROMOTION_LOGISTICS]后攻击[COLOR_POSITIVE_TEXT]概率[ENDCOLOR]获得")
  end
  --旗舰锁定
  if (sPromotion == "PROMOTION_FLAGSHIP_LOCKING" and sCombatClass == "UNITCOMBAT_SUBMARINE") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("[COLOR_POSITIVE_TEXT]累计[ENDCOLOR]攻击3次敌军旗舰[ICON_PROMOTION_NAVAL_CAPITAL_SHIP]/航母[ICON_PROMOTION_CARRIER_UNIT]后获得")
  end
  --齐射强化
  if (sPromotion == "PROMOTION_SHIPBORNE_GUN_SALVO_2" and sCombatClass == "UNITCOMBAT_NAVALRANGED") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("俾斯麦号[COLOR_POSITIVE_TEXT]专属[ENDCOLOR]晋升!")
  end
  --精通强化
  if (sPromotion == "PROMOTION_EXP_ADAPT_4" and sCombatClass == "UNITCOMBAT_GUN") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("同时拥有适应三级[ICON_PROMOTION_QY_EXPROMOTION_45]和技能三级[ICON_PROMOTION_DOGFIGHTING_3]且击败敌军[COLOR_POSITIVE_TEXT]精英单位[ENDCOLOR]后获得")
  end
  --破坏射击
  if (sPromotion == "PROMOTION_CANNON_DAMAGE_SHOT" and sCombatClass == "UNITCOMBAT_SIEGE") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("需要拥有[COLOR_POSITIVE_TEXT]间接火力[ENDCOLOR][ICON_PROMOTION_INDIRECT_FIRE]才可选择")
  end
  --冲击4级、操练4级
  if ((sPromotion == "PROMOTION_SHOCK_4" or sPromotion == "PROMOTION_DRILL_4") and sCombatClass == "UNITCOMBAT_MELEE") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("武僧[COLOR_POSITIVE_TEXT]专属[ENDCOLOR]晋升!")
  end
  --天空神剑
  if (sPromotion == "PROMOTION_AIR_EXCALIBUR" and sCombatClass == "UNITCOMBAT_ARCHER") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("单位攻击时[COLOR_POSITIVE_TEXT]一次性受到[ENDCOLOR]高额伤害并存活后获得")
  end
  --向死而生
  if (sPromotion == "PROMOTION_AIR_TO_DIE_AND_LIVE" and sCombatClass == "UNITCOMBAT_ARCHER") then
    sToolTip = sToolTip .. "[NEWLINE][NEWLINE]" .. Locale.ConvertTextKey("单位[COLOR_POSITIVE_TEXT]复活[ENDCOLOR]后获得")
  end
...
function PopulateClassList()
  ...
  local sQuery = "SELECT x.CombatClass, x.IsDefault, x.Description, x.DefaultUnitType, x.BasePromotion FROM UnitCombatInfosEx x"   -- UndeadDevel: we just use our own class table
  for row in DB.Query(sQuery) do
    table.insert(g_ClassList, {CombatClass=row.CombatClass, TextKey=row.Description, UnitType=row.DefaultUnitType, BasePromotion = row.BasePromotion})
...
function OnSelectCombatClass(iIndex)
  ...
  PlacePromotions(nil, g_ClassList[iIndex].CombatClass, iLeftMargin + iDropDownSizeX, iCentreLine, g_ClassList[iIndex].BasePromotion)
end
